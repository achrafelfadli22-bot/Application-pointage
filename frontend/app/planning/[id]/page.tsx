                    ...
                    <tr key={index} className="border-t border-borderSoft">
                      <td className="p-2">
                        {canManage ? <select disabled={readonly} value={line.projectId} onChange={(event) => patchLine(index, { projectId: event.target.value, siteId: '', userId: '' })} className="h-9 w-48 rounded-md border border-borderSoft px-2 disabled:bg-grayCard">
                          <option value="">Sélectionner un projet</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.code} - {project.name}</option>)}
                        </select> : <span className="font-medium text-bodyText">{line.projectLabel}</span>}
                      </td>
                      <td className="p-2">{canManage ? <select disabled={readonly || !line.projectId} value={line.siteId} onChange={(event) => patchLine(index, { siteId: event.target.value, userId: '' })} className="h-9 w-48 rounded-md border border-borderSoft px-2 disabled:bg-grayCard"><option value="">Sélectionner un site</option>{sites.filter((item) => item.project?.id === line.projectId).map((item) => <option key={item.id} value={item.id}>{siteLabel(item)}</option>)}</select> : <span>{line.siteLabel}</span>}</td>
                      <td className="p-2">{canManage ? <select disabled={readonly || !line.siteId} value={line.userId} onChange={(event) => selectEmployee(index, event.target.value)} className="h-9 w-44 rounded-md border border-borderSoft px-2 disabled:bg-grayCard"><option value="">Sélectionner un employé</option>{employeesForSite(line.siteId).map((person) => <option key={person.id} value={person.id}>{person.firstName} {person.lastName}</option>)}</select> : <span>{line.userLabel}</span>}</td>
                      <td className="p-2"><input disabled={readonly} value={line.taskName} onChange={(event) => patchLine(index, { taskName: event.target.value })} placeholder="Description (optionnelle)" className="h-9 w-40 rounded-md border border-borderSoft px-2 disabled:bg-grayCard" /></td>
                      {days.map((day) => {
